...
function main() {
  ...
  window.addEventListener('keydown', function onWindowKeydown(e) {
    ...
    if (shiftKey && key === Key['9']) {
      ...
      [Key.Multiply, Key.Add, Key.Subtract, Key.Divide].includes(key)
    ) {
      store.dispatch(ACTION_UPDATE_OPERATOR, {
...
const { MathParser, isOperator, operators } = (function(module) {
  ...
  const RE_NUMERIC_LITERAL = (exports.RE_NUMERIC_LITERAL = /^(?:(?:0x[0-9a-fA-F]+|0X[0-9a-fA-F]+)|(?:0[oO][0-7]+)|(?:0[bB][01]+)|(?:(?:0|[1-9](?:[0-9]+)?)\.(?:[0-9]+)?|\.[0-9]+|(?:0|[1-9](?:[0-9]+)?))(?:[eE](?:[-+][0-9]+))?)/)
  ...
  const punctuation = (exports.punctuation = ['(', ')', ...operators])

  // All the operators, sorted by longest string length
  const RE_PUNCTUATION = (exports.RE_PUNCTUATION = new RegExp(
    '^(?:' +
      punctuation
        .slice(0)
  ...
  const isOperator = (exports.isOperator = str => operators.includes(str))
  ...
  const Lexer = (exports.Lexer = class Lexer {
    ...
    lex() {
      const token = null || this.getPunctuationToken() || this.getNumericToken()
